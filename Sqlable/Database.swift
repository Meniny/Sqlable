...
//  Sqlable
//
//  Created by Elias Abel on 28/07/15.
...
public class SQLiteDatabase {
	...
	func run<T, SQLReturn>(_ statement: SQLStatement<T, SQLReturn>) throws -> Any {
		...
		case .select:
			...
			loop: while true {
				switch sqlite3_step(handle) {
				case SQLITE_ROW:
					rows.append(try T(row: SQLReadRow(handle: handle, tablename: T.tableName)))
					continue
				case SQLITE_DONE: break loop
				case SQLITE_BUSY:
					if waits == 0 { try throwLastError(db) }
			...
			if statement.single {
				if let first = rows.first {
					returnValue = SQLSingleResult.result(first)
				} else {
					returnValue = SQLSingleResult<T>.noResult
				}
			} else {
				returnValue = rows
			}
